...
import { AppStyle, setOverFlowX, flexShrink, width, cursorPointer, height, radius, border, borderWidth, margin, flexCenterInParent, flexHori } from "../../AppStyle"
...
export default function AddImage() {
    ...
    return <Rows style={AppStyle(setOverFlowX())}>
        <AddImageItem onChangeEvent={event => {
            console.log({ event })
        ...
        <Rows>
            {
                listImage.map((imageItem, index) =>
                    <ImageItem key={imageItem} position={0} item={imageItem} onItemDelete={() => {
...
const AddImageItem: FC<AddImageItemProps> = (props) => {
    return <div>
        ...
        />
        <label htmlFor="file"
            style={AppStyle(flexHori(), flexShrink(0), width(150), cursorPointer(), height(200), radius(8), border(Colors.color_8A8A8F), borderWidth(1), margin(12), flexCenterInParent())}>
...
const ImageItem: FC<ImageItemProps> = (props) => {
    return <div
        className="container" style={AppStyle(width(150), height(200))}
        onClick={() => {
